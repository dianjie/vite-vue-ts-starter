...
const transform: AxiosTransform = {
  ...
  requestInterceptors: (config, options) => {
    if ((config as Recordable)?.requestOptions?.authorization !== false) {
      ;(config as Recordable).headers.Authorization = `Basic ${btoa(
  ...
  responseInterceptorsCatch: (error: any) => {
      ...
      try {
        ...
        if (errMessage) {
          if (errorMessageMode === 'dialog') {
            ...
            ElMessage({ type: 'error', message: errMessage })
          }
    ...
    return backoff.then((config) => request.request(config as AxiosRequestConfig))
  }
}
